...
- Decoration extractor parity: `extractAllDecorationRanges()` must preserve `extractHighlightRanges()` behavior for `==...==` inside any CriticMarkup span (including `{==...==}`, `{++...++}`, `{--...--}`, etc.); no CriticMarkup span type suppresses nested format highlights
