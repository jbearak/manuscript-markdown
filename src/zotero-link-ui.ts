 ...
 *  nothing is unmatched, so no stale file is written.  `eol` is the source
 *  file's line ending, so the export matches it. */
export function buildUnmatchedBibliography(
  ...
  const chunks: string[] = [
    '% Entries "Link Bibliography to Zotero" could not match in ' + libraryLabel + '.',
  ...
  for (const decision of unmatched) {
    chunks.push('% ' + decision.entry.key + ': ' + UNMATCHED_PROSE[decision.reason]);
